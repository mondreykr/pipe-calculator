...
# Open Questions
- **Is a GitHub Pages URL an acceptable home for ASME-derived values?** Verified 2026-08-18 that `enercorp/bom-tool` is a private repository whose Pages site answers an anonymous request from the open internet with HTTP 200. The site is unlisted, not access-controlled. Publishing roughly fifteen Table A-1 stress values there is a different act from putting them on an internal server, and it may need a decision from above engineering.
- **Should this repository move to the `enercorp` org?** It currently sits at `mondreykr/pipe-calculator`; `bom-tool` sits under `enercorp`. Transferring would match the pattern and survive any one person leaving.
